...
function Bomb(){
  var self = this;
  self.canvas = document.getElementById("banner-canvas");
  ...
  self.px = (self.canvas.width / 4) + (Math.random() * self.canvas.width / 2);
  self.py = self.canvas.height;
...
function Controller(){
  ...
  function setSpeedParams(){
    ...
    while(heightReached < self.canvas.height && vy >= 0){
      vy += gravity;
      heightReached += vy;
    }
